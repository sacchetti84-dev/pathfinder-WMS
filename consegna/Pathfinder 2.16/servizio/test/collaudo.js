...
(async () => {
  ...
  /* ══ 2.13 · L'ATTESA DELL'AVVIO DI POSTGRESQL, PROVATA DA FERMA ═══════
     ...
     magazzino fermo, copiata nel pacchetto e MAI RIPORTATA NEL SORGENTE:
     la 2.13 la riporta, e siccome la riporta la copre. Si prova da ferma,
     con un orologio finto e un sonno finto — accendere un PostgreSQL e
     spegnerlo a meta' non e' una prova, e' una coincidenza. */
  const { DriverPostgres, siRiprova, attesaPrima } = require('../lib/driver-postgres.js');

  ok('un database che sta ancora salendo si aspetta',
     ['ECONNREFUSED', 'ECONNRESET', 'ETIMEDOUT', '57P03', '57P01', '57P02', '08006']
       .every(code => siRiprova({ code })));
  ok('una password sbagliata non si aspetta', siRiprova({ code: '28P01' }) === false);
  ok('un database che non esiste non si aspetta', siRiprova({ code: '3D000' }) === false);
  ok('ne un permesso che manca, ne un host che non si risolve',
     siRiprova({ code: '42501' }) === false && siRiprova({ code: 'ENOTFOUND' }) === false);
  /* `pg` non sempre porta lo SQLSTATE: allora parla il messaggio. */
  ok('senza codice, il messaggio basta a distinguere',
     siRiprova({ message: 'password authentication failed for user "pathfinder"' }) === false
       && siRiprova({ message: 'connect ECONNREFUSED 127.0.0.1:5432' }) === true);
  /* UN CODICE MAI VISTO SI ASPETTA, e non e' un dettaglio: e' la scelta che
     questa correzione fa. Al peggio si perde un minuto e mezzo una volta;
     il verso opposto e' il difetto del 28/08. */
  ok('un codice mai visto si aspetta, non si scarta', siRiprova({ code: 'XX999' }) === true);

  ok('la scala dell attesa raddoppia e si ferma a otto secondi',
     [2, 3, 4, 5, 6, 7].map(attesaPrima).join(',') === '1000,2000,4000,8000,8000,8000',
     [2, 3, 4, 5, 6, 7].map(attesaPrima).join(','));

  /* L'attesa vera, con l'orologio in mano. Il pool finto risponde come gli
     si dice, un elemento per tentativo: `null` vuol dire «il server c'e'». */
  const attendiCon = async (risposte, limiteMs = 90_000) => {
    let adesso = 0, i = 0;
    const dormito = [];
    const finto = { pool: { query: async () => {
      const r = risposte[Math.min(i++, risposte.length - 1)];
      if (r) throw Object.assign(new Error('finto'), r);
      return { rows: [] };
    } } };
    const tentativi = await DriverPostgres.prototype._attendiIlServer.call(
      finto, limiteMs, () => adesso, async (ms) => { dormito.push(ms); adesso += ms; });
    return { tentativi, dormito };
  };

  const salito = await attendiCon([{ code: 'ECONNREFUSED' }, { code: '57P03' }, null]);
  ok('il database che sale al terzo colpo viene raggiunto', salito.tentativi === 3,
     'tentativi ' + salito.tentativi);
  ok('e fra un tentativo e l altro si e dormito davvero',
     salito.dormito.join(',') === '1000,2000', salito.dormito.join(',') + ' ms');

  const subito = await attendiCon([null]);
  ok('un database gia in piedi non fa aspettare nessuno',
     subito.tentativi === 1 && subito.dormito.length === 0);

  let arreso = null;
  try { await attendiCon([{ code: 'ECONNREFUSED' }]); } catch (e) { arreso = e; }
  ok('finito il tempo si smette, e il messaggio dice che e finito il tempo',
     arreso !== null && /non ha risposto entro 90s/.test(arreso.message),
     arreso ? arreso.message : 'non ha lanciato');

  let credenziale = null;
  try { await attendiCon([{ code: '28P01' }]); } catch (e) { credenziale = e; }
  ok('una password sbagliata si scopre subito, senza aspettare novanta secondi',
