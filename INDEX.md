...
| **86** | ⚠️ **CHE QUELLE STAMPANTI ABBIANO UNA PRESA DI RETE NON È STATO VERIFICATO, E SE NON CE L'HANNO LA FUNZIONE NON LE RAGGIUNGE.** Tutta la 2.19 poggia su una cosa sola: che la stampante ascolti su una porta TCP. Le desktop Zebra della fascia ZD200 e le precedenti simili escono spesso **con la sola USB**, e l'Ethernet è un'opzione che dipende dal modello esatto. Se sono USB, il servizio non ha nessuno a cui parlare — e il difetto non si vede finché qualcuno non preme «Stampa» in corsia | **Si guarda il retro della macchina**: se c'è una presa RJ45 accanto alla USB, la strada c'è. In alternativa si tiene premuto **FEED** all'accensione finché non stampa il foglio di configurazione, che porta indirizzo IP e porte. **Se sono USB-only, la funzione non va riscritta**: si mette un **print server esterno** — un box USB→Ethernet che espone la 9100 — e il codice lo raggiunge com'è. La porta **6101** è già nell'elenco ammesso proprio per i print server più vecchi. Da chiarire **prima** della voce 83: senza rete, non c'è niente da provare |
