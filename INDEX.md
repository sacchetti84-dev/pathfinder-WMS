...
|---|---|---|
| **80** | 🔴 **SEI EXPORT JSON TRACCIATI PORTANO IMPRONTE PIN DI PERSONE VERE.** È **la voce 72 che si ripete su un'altra estensione**: il `.gitignore` del 31/08 ha chiuso `*.dump`, `*.sql`, `*.sqlite`, `*.bak` — e un **export JSON è un database**. In `ARCHIVIO\BACKUP E FILE DI TEST\`: `warehouse-mapper-2026-08-20_GIACENZE REALI IN COLLI.json` (ANSA) — **che è la base di prova di Andrea, quella da cui riparte ogni volta** — più `pathfinder-backup-2026-08-25-1845.json` (ANSA), `14082026_warehouse-mapper-2026-08-14.json` e `wm-stato-2026-08-18.json` (ANDS, BABB, EFBR), `08082026_wm-test-v2.8.0.json` (ANDS, GV, MR, TL), `08082026_wm-stato-2026-08-08.json` (AS). Sono **tracciati adesso**, non solo nella storia | **Due decisioni, e sono di Andrea.** (1) **Togliere le impronte dai file**: `pin_hash`, `pin_salt`, `rec_hash`, `rec_salt` non servono a una prova — un backup senza PIN riapre la finestra del primo avvio, che è **il modo giusto** di partire su una versione nuova. Prima però va verificato che non rompa il modo di lavorare di Andrea, che quei PIN forse li usa per entrare. (2) **La storia**: quei byte ci sono da agosto, e valgono le considerazioni della voce 78. **E la lezione va imparata come si deve**: non un'altra riga nel `.gitignore`, ma **una prova** che fallisce se un file tracciato porta un'impronta. Una regola scritta in prosa e un filtro scritto per estensione non sono la stessa regola, e questa è la seconda volta |
| **79** | 🔴 **DALLA 1.4 ALLA BETA: IL SALTO NON È MAI STATO PROVATO.** Il magazzino vero gira la **1.4** su un'altra macchina — §0. È un **HTML unico da 1,54 MB**, dati in **IndexedDB via Dexie** (`WarehouseMapperDB`), backup su OPFS, `exportAll`/`importAll` con `_format` **`warehouse-mapper-v1.5`** — lo stesso che dichiara la beta. **La strada quindi esiste**: si esporta dal browser del magazzino e si importa nella beta. Verificato il 02/09 che due export veri dell'epoca — `_appVersion` **1.6** e **1.1.0** — si lascino **chiavare dallo schema di oggi**: ogni collezione trova la sua chiave primaria, e quelle assenti restano com'erano come vuole §8. **Ma nessuno l'ha mai fatto girare da capo a fondo** | **Un banco che parte da un export vero e dimostra che non si perde niente.** Prende `14082026_warehouse-mapper-2026-08-14.json`, lo importa in una beta vuota e conta: articoli, giacenze, lotti, compiti, documenti, operatori. Tre cose da guardare in particolare: le **UM** (la 1.4 non le aveva come oggi), i **colli** (`packs` è nato dopo), e i **PIN**, che lì sono SHA-256 e la 2.10 riscrive in scrypt al primo accesso. Finché questo banco non è verde, «beta» è una parola |
