...
| **44** | **IL REGISTRO DEI MOVIMENTI VIENE SVUOTATO DAL RECUPERO DI UN BACKUP AUTOMATICO — causa trovata il 25/08, e la catena è completa.** `Store.writeOPFSBackup()` è la copia automatica locale (ogni 7 giorni, ne tiene 4) e chiama `exportAll({ includeMovLog: false })`: **il file non porta il registro**, e il codice lo sa — ci scrive dentro `_partial: 'senza registro movimenti'`. Quando quel file rientra, `importAll(data, 'overwrite')` fa `clearMany(COLLEZIONI_EXPORT)` — che **svuota anche `mov_log`** — e poi riempie solo le collezioni che il pacchetto porta: `if (!Array.isArray(righe) \|\| !righe.length) continue`. Il registro esce svuotato e non viene mai riempito. **`_partial` è scritto in un punto solo e non lo legge nessuno**: nessun avviso, nessuna conferma, niente. Lo svuotamento è coerente con l'intenzione scritta sopra il `clearMany` — «il ripristino deve lasciare il magazzino nello stato del file» — ma quell'intenzione vale per lo *stato*, e il registro non è stato: è storia che si accumula | **La causa è chiusa, la correzione no: voce 45.** Il conteggio quadra con le date — 19/08 → 253, 20/08 → 1, 22 e 23/08 → 0, e il 20/08 è il giorno dei ripristini fatti per raddrizzare le tredici righe storte |
| **45** | **IL RIPRISTINO DEVE SMETTERE DI CANCELLARE IL REGISTRO** — è la correzione della voce 44. Due cose, e sono diverse: (a) `importAll` non deve svuotare `mov_log` quando il pacchetto **non lo porta** — per le altre collezioni svuotare è giusto, perché sono stato e devono combaciare con le giacenze, ma il registro è storia che si accumula e un pacchetto che tace non è un pacchetto che dice «vuoto»; (b) `_partial` va **letto**, e chi preme «recupera» deve vedere scritto che quel file non contiene il registro **prima** di premere | Tocca la rotta di ripristino, che è il gesto più pericoloso dell'applicativo: prove nuove sui due rami, e il diff si guarda prima di consegnare |
...
| # | Cosa | Cosa manca |
|---|---|---|
| **18** | **Le sigle `DP` e `AS`, e la premessa che è cambiata.** Andrea, 25/08: «sono operatori fittizi, eliminale». Misurato lo stesso giorno, e non è così semplice: **`DP` È in anagrafica operatori**, con `first_name` e `last_name` **vuoti**, ruolo `operator`, `pin_salt: null` — cioè un operatore creato e mai completato, non una sigla digitata a mano. `AS` non c'è. E dei **16 movimenti** che portano quelle firme, solo **6 sono di prova** (articoli `123`, `123456`, lotti `qwert` e `hjk`): gli altri **10 sono PICK del 07/08 su articoli, lotti e ubicazioni VERI** — `6000242`, `6000296`, `6000004`, `6000006`, `6000149`, `6001335`, `6000002`, `6000049`, `6000401`, `6000286`, uno col «Lotto forzato». Quella merce si è mossa davvero | **Cancellare la sigla e cancellare il movimento sono due cose diverse, e la seconda toglie storia vera.** Serve la decisione su quale delle due, e su cosa fare dei 10 PICK. Vedi anche la voce 46 |
| **46** | **L'ANAGRAFICA OPERATORI È STATA SOSTITUITA IN BLOCCO, e nessuno l'ha scritto.** Al 19/08 c'erano `ANDS` (Andrea Sacchetti, leader), `ANAD` (ANDREA SACCHETTI, admin), `BABB` («babbo natale») ed `EFBR` («elfo brutto»); oggi ci sono `ANSA` (Andrea Sacchetti, leader) e `DP` (senza nome, senza PIN). **Nessuna delle quattro di prima esiste più**, ed è coerente con lo svuotamento della voce 44: il ripristino sostituisce anche `operators`. Conseguenza sul recupero dei 253 movimenti: **231 di quelli portano firme che oggi non esistono** — `ANDS` 144, `ANAD` 87 — e due di quelle sigle erano dichiaratamente di prova | Va deciso **prima** del recupero: o le sigle vecchie rientrano in anagrafica come operatori storici disattivati, o il registro recuperato porta 231 firme senza un nome dietro, cioè il difetto della voce 18 moltiplicato per quindici |
| **47** | **L'anagrafica dei mittenti va pulita** — Andrea, 25/08. Oggi in `recipients` ci sono **2 righe**, `biotech` ed `eurospin`, tutte e due minuscole, con `destinations` **vuoto**, create il 24/08 sera da `ANSA`: sono le prove del banco, non clienti. Nel backup del 19/08 ce n'erano **4, tutte e quattro chiamate «BIOTECH SRL»**, tutte allo stesso indirizzo «VIA NUOVA, 41032»: lo stesso cliente inserito quattro volte. Un DDT che punta a una di quelle quattro non è distinguibile da uno che punta a un'altra | Le due righe di oggi si cancellano senza pensarci. Le quattro del backup contano solo se il recupero della voce 44 le riporta indietro: allora vanno **fuse in una**, non cancellate, perché i DDT già emessi le nominano |
