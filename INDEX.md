...
| **44** | **IL REGISTRO DEI MOVIMENTI È STATO SVUOTATO TRE VOLTE, E NESSUNO L'HA SCRITTO.** Contati il 25/08 sulle copie serali in `C:\Pathfinderackup\`, una per una: **19/08 → 253 movimenti** (`_id` 60–335, il registro che il guardiano lesse); **20/08 → 1** (`_id` 723: fra le due sere ne furono scritti altri ~390 e poi spazzati tutti); **22/08 → 0**; **23/08 → 0**; **24/08 → 20**; **25/08 → 20** (`_id` 752–771, tutti del 24/08 sera, tutti a firma `ANSA`, tutti in `MAG1`). Il servizio **non ha nessuna potatura automatica** — cercata in `pathfinder-server.js` e in `lib/db.js`, non esiste — quindi è stato un gesto, e il candidato è il `clear` che accompagna il recupero di un backup (`/api/clear`). Il 20/08 è il giorno delle tredici righe raddrizzate «in produzione dopo una copia» | **È la voce più grave aperta, e viene prima della 33**: correggere chi scrive un registro che qualcosa svuota non serve a niente. La promessa è che il registro si tenga **sei anni**: oggi tiene una sera. **I 253 movimenti del 19/08 non sono persi** — stanno in `pathfinder-2026-08-19.db`, e da lì si recuperano. Prima si capisce **quale gesto li cancella**, poi si decide se il recupero si fa |
