...
| # | Cosa | Cosa manca |
|---|---|---|
| **18** | **`DP` è l'ultima firma senza un nome dietro** — 10 movimenti sui 266. Andrea, 25/08: «dai un nome a DP, butta i 6 di prova». **Fatta la seconda metà**: gli otto movimenti sugli articoli di prova `123` e `123456` sono rimasti fuori dal recupero — erano sei di `DP` e `AS`, più due del 19/08 firmati `ANDS` e `ANAD` che muovevano lo stesso articolo finto. `AS` non compare più da nessuna parte. Restano i **10 PICK veri del 07/08** firmati `DP`, su articoli, lotti e ubicazioni esistenti | **Manca il nome.** `DP` è in anagrafica con `first_name` e `last_name` vuoti e `pin_salt: null`: si completa in un minuto da Configurazione → Operatori, o lo si dice e lo scrive un agente |
