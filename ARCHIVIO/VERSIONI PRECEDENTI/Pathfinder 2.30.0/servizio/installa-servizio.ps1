...
#
# 2.12.1 — E UN MINUTO DOPO, NON SUBITO. Il 28/08 questa attività è partita
# 18 secondi dopo l'accensione, ha trovato PostgreSQL che stava ancora
# facendo il recovery, e il servizio è uscito con 1 come deve. Il magazzino
# è rimasto giù una giornata. La difesa vera sta nel servizio, che adesso
# aspetta il database invece di arrendersi al primo no; questo minuto è la
# seconda, e costa un minuto di attesa su una macchina che si accende.
...
# Se cade, riparte: cinque tentativi a due minuti di distanza. Nessun limite
# di durata, perché deve restare acceso quanto la macchina.
#
# 2.12.1 — SU QUESTA RIGA NON SI FA AFFIDAMENTO, ed è la lezione del 28/08.
# «Riavvia in caso di errore» dell'Utilità di pianificazione ripesca le
# attività che NON RIESCONO A PARTIRE, non quelle il cui processo esce con
# un codice diverso da zero: quel giorno il conteggio non è scattato e
# `LastRunTime` è rimasta all'unico tentativo. Resta perché copre il caso
# che copre — un processo ucciso, una macchina in affanno — ma chi legge
# sappia che non è lei a garantire l'accensione.
