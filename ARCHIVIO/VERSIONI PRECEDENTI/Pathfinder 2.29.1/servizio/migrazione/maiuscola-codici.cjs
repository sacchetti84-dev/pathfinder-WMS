...
/* I CODICI GIA' SCRITTI, RADDRIZZATI IN MAIUSCOLO — 2.6.
   ...
   solo a mettere in pari quello che era stato scritto prima.
     ...
     node server/migrazione/maiuscola-codici.cjs --da "banco\\db\\pathfinder-<data>.db"
     node server/migrazione/maiuscola-codici.cjs --da "..." --scrivi
...
const { NAMES, COLLECTIONS, MAIUSCOLE, normalizza, materialize } = require('../lib/schema');
...
function assicuraFirma(db, ora) {
  ...
  const op = normalizza('operators', {
    ...
    notes: "Non e una persona: e la firma delle fusioni scritte da server/migrazione/maiuscola-codici.cjs. Disattivata e senza PIN, non puo operare: esiste perche il registro la nomina (voci 18 e 46).",
  });
